...
_MODEL_PREF = os.environ.get("OLLAMA_MODEL", "qwen2.5:32b-instruct-q4_K_M")


def _param_size(name: str) -> int:
...
def _resolve_model() -> str:
    """Pick a model that is actually installed in Ollama, so the app works on
    whatever the host has (qwen 3b / 7b / 32b ...). Order: the preferred tag if
    present, else the largest installed qwen2.5, else the first model."""
    try:
        names = [m["name"] for m in
                 requests.get(_OLLAMA_BASE + "/api/tags", timeout=5).json().get("models", [])]
    except Exception:
        return _MODEL_PREF
    if not names:
        return _MODEL_PREF
    exact = next((n for n in names if n == _MODEL_PREF or n.startswith(_MODEL_PREF)), None)
    if exact:
        return exact
    qwen = [n for n in names if "qwen2.5" in n.lower()]
    return max(qwen, key=_param_size) if qwen else names[0]
...
def _call_llm(resume: str) -> list[dict]:
    ...
    resp = requests.post(
        OLLAMA_URL,
        json={"model": _resolve_model(), "prompt": prompt, "format": "json", "stream": False,
              "keep_alive": "15m",
